...
export default function Page() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const result = await fetchAdminAudit(params);
      if (cancelled) return;
      if (result.ok) {
  ...
  return (
    <div data-testid="admin-audit-page">
      <AdminPageHeader title="Audit Logs" description="Administrative metadata view with privacy-safe fields." />
      <FilterBar>
        <FilterInput
          placeholder="Filter…"
          value={q}
        ...
        <FilterSelect
          ...
          data-testid="admin-audit-status-filter"
        >
        ...
        columns={[
          {
            ...
            render: (r: Record<string, unknown> & { id: string }) =>
              String(r.action ?? r.safe_summary ?? r.filename ?? r.title ?? r.tool_name ?? "—"),
          },
          {
            key: "owner",
            header: "Owner / Actor",
          ...
          },
          {
            ...
            header: "Status / Type",
            render: (r: Record<string, unknown> & { id: string }) => (
              <StatusBadge status={String(r.action ?? r.target_type ?? "unknown")} />
            ),
          },
          {
            key: "created",
